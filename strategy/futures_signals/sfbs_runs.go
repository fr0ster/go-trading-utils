	...
	if side == futures.SideTypeSell {
		...
		if order.GetUpPrice() == 0 {
			...
			if (pair.GetUpBound() == 0 || upPrice <= pair.GetUpBound()) &&
				...
				upOrder, err := createOrderInGrid(pairProcessor, futures.SideTypeSell, futures.OrderTypeLimit, quantity, upPrice, false)
				if err != nil {
					printError()
					return err
				}
		...
		if ok && downPrice.GetOrderId() == 0 && downPrice.GetQuantity() <= 0 {
			...
			downOrder, err := createOrderInGrid(pairProcessor, futures.SideTypeBuy, futures.OrderTypeLimit, quantity, order.GetDownPrice(), false)
			if err != nil {
				printError()
				return err
			}
		...
		if order.GetDownPrice() == 0 {
			...
			if (pair.GetLowBound() == 0 || downPrice >= pair.GetLowBound()) &&
				...
				utils.ConvStrToFloat64(risk.IsolatedMargin) <= pair.GetCurrentPositionBalance() &&
				locked <= pair.GetCurrentPositionBalance() {
				downOrder, err := createOrderInGrid(pairProcessor, futures.SideTypeBuy, futures.OrderTypeLimit, quantity, downPrice, false)
				if err != nil {
					printError()
					return err
				}
		...
		if ok && upRecord.GetOrderId() == 0 && upRecord.GetQuantity() <= 0 {
			// Створюємо ордер на продаж
			upOrder, err := createOrderInGrid(pairProcessor, futures.SideTypeSell, futures.OrderTypeLimit, quantity, order.GetUpPrice(), false)
			if err != nil {
				printError()
				return err
			}
...
func loadConfig(pair *pairs_types.Pairs, config *config_types.ConfigFile, pairProcessor *PairProcessor) (err error) {
	baseValue, err := pairProcessor.GetBaseBalance()
...
func initFirstPairOfOrders(
	...
	sellQuantity := round(pair.GetCurrentPositionBalance()*pair.GetLimitOnTransaction()/sellPrice, stepSizeExp)
	...
	buyQuantity := round(pair.GetCurrentPositionBalance()*pair.GetLimitOnTransaction()/buyPrice, stepSizeExp)
...
func marginBalancing(
	...
	if config.GetConfigurations().GetBalancingOfMargin() && utils.ConvStrToFloat64(risk.PositionAmt) != 0 {
		...
		freeOut, _ = pairProcessor.GetBaseBalance()
	} else {
		freeOut = free
	}
	return
	...
	if config.GetConfigurations().GetObservePositionLoss() {
		...
		if len(openOrders) == 1 {
			...
			if sellDeltaPercent > config.GetConfigurations().GetSellDeltaLoss() || buyDeltaPercent > config.GetConfigurations().GetBuyDeltaLoss() {
				...
				_, _, err = initFirstPairOfOrders(pair, price, tickSizeExp, stepSizeExp, pairProcessor)
				if err != nil {
					printError()
					return err
				}
...
func RunFuturesGridTrading(
	...
	sellOrder, buyOrder, err := initFirstPairOfOrders(pair, currentPrice, tickSizeExp, stepSizeExp, pairProcessor)
	if err != nil {
		return err
	}
	// Ініціалізація гріду
...
func getPrice(
	...
	if utils.ConvStrToFloat64(risk.PositionAmt) < 0 {
		if pair.GetUpBound() != 0 {
			...
			stepUp := freeUp * pair.GetLimitOnTransaction()
			deltaUp = stepUp/entryPrice - pair.GetSellDelta()
		} else {
			deltaUp = 0
		}
		...
		if pair.GetLowBound() != 0 {
			...
			stepDown := freeDown * pair.GetLimitOnTransaction()
			deltaDown = stepDown/entryPrice - pair.GetBuyDelta()
		} else {
			deltaDown = 0
		}
...
func createNextPair_v0(
	...
	if pair.GetUpBound() != 0 && upPrice <= pair.GetUpBound() {
		if positionVal >= -pair.GetCurrentPositionBalance() {
			...
			_, err = createOrderInGrid(pairProcessor, futures.SideTypeSell, futures.OrderTypeLimit, correctedQuantityUp, upPrice, false)
			if err != nil {
				printError()
				return
			}
	...
	if pair.GetLowBound() != 0 && downPrice >= pair.GetLowBound() {
		if positionVal <= pair.GetCurrentPositionBalance() {
			...
			_, err = createOrderInGrid(pairProcessor, futures.SideTypeBuy, futures.OrderTypeLimit, correctedQuantityDown, downPrice, false)
			if err != nil {
				printError()
				return
			}
...
func timeProcess(
	...
	if config.GetConfigurations().GetBalancingOfMargin() ||
		...
		free, _ = pairProcessor.GetBaseBalance()
	...
	err = positionLossObservation(config, pair, risk, pairProcessor, currentPrice, tickSizeExp, stepSizeExp)
	if err != nil {
		return err
	}
	return
...
func RunFuturesGridTradingV3(
	...
	// Створюємо стрім подій
	_, pairProcessor, err = initRun(config, client, pair, quit)
	if err != nil {
		return err
	}
	err = loadConfig(pair, config, pairProcessor)
	...
	_, _, err = initFirstPairOfOrders(pair, initPrice, tickSizeExp, stepSizeExp, pairProcessor)
	if err != nil {
		return err
	}
	// Стартуємо обробку ордерів
...
func createNextPair_v1(
	...
	getQuantity := func(risk *futures.PositionRisk, upPrice, downPrice float64, sizeSizeExp int) (upQuantity, downQuantity float64) {
		...
		if utils.ConvStrToFloat64(risk.PositionAmt) < 0 {
			...
			upQuantity = round(freeUp*pair.GetLimitOnPosition()*pair.GetLimitOnTransaction()/upPrice, sizeSizeExp)
			...
			downQuantity = round(freeDown*pair.GetLimitOnPosition()*pair.GetLimitOnTransaction()/downPrice, sizeSizeExp)
			...
			upQuantity = round(freeNew*pair.GetLimitOnPosition()*pair.GetLimitOnTransaction()/upPrice, sizeSizeExp)
			downQuantity = round(freeNew*pair.GetLimitOnPosition()*pair.GetLimitOnTransaction()/downPrice, sizeSizeExp)
		}
		return
	}
...
func RunFuturesGridTradingV4(
	...
	logrus.Debugf("Futures %s: Start Order Status Event", pair.GetPair())
	maintainedOrders := btree.New(2)
	streamStart(client, getCallBack(config, client, pair, pairProcessor, tickSizeExp, stepSizeExp, quit, maintainedOrders))
	<-quit
	err = loadConfig(pair, config, pairProcessor)
	if err != nil {
		printError()
