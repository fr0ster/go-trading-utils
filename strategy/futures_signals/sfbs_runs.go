...
func RunFuturesGridTrading(
	...
	initOrderInGrid := func(side futures.SideType, quantity, price float64) (order *futures.CreateOrderResponse, err error) {
		for {
	...
	sellOrder, err := initOrderInGrid(futures.SideTypeSell, quantity, price*(1+pair.GetSellDelta()))
	if err != nil {
		stopEvent <- os.Interrupt
		return err
	}
	...
	buyOrder, err := initOrderInGrid(futures.SideTypeBuy, quantity, price*(1-pair.GetSellDelta()))
	if err != nil {
		stopEvent <- os.Interrupt
		return err
	}
	...
	for {
		...
		case event := <-pairProcessor.GetOrderStatusEvent():
			...
			if upOrder.GetOrderId() == 0 {
				logrus.Debugf("Futures %s: Set Sell order on price %v", pair.GetPair(), upOrder.GetUpPrice())
			...
			if downOrder.GetOrderId() == 0 {
				logrus.Debugf("Futures %s: Set Buy order on price %v", pair.GetPair(), downOrder.GetDownPrice())
