...
type (
	PairBookTickersObserver struct {
		...
		stream    *spot_streams.BookTickerStream
		event     chan bool
		stop      chan os.Signal
		deltaUp   float64
		deltaDown float64
...
func (pp *PairBookTickersObserver) GetStream() *spot_streams.BookTickerStream {
	return pp.stream
}

func (pp *PairBookTickersObserver) StartStream() *spot_streams.BookTickerStream {
	if pp.stream == nil {
		if pp.data == nil {
		...
		pp.stream = spot_streams.NewBookTickerStream(pp.pair.GetPair(), 1)
		pp.stream.Start()
...
func (pp *PairBookTickersObserver) StartUpdateGuard() chan bool {
	if pp.event == nil {
		...
		pp.event = spot_handlers.GetBookTickersUpdateGuard(pp.data, pp.stream.GetDataChannel())
	}
	return pp.event
}
