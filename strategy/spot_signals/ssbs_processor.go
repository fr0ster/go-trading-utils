	...
	go func() {
		for {
			...
			case <-triggerEvent: // Чекаємо на спрацювання тригера на зміну bookTicker
				...
				commission := GetCommission(account)
				minAsk := depths.GetAsks().Min()
				maxBid := depths.GetBids().Max()
				// Ціна купівлі
...
func (pp *PairProcessor) StartBuyOrSellByBookTickerSignal() {
	...
	pp.buy, pp.sell = pp.BuyOrSellByBookTickerSignal(pp.account, pp.bookTickers, pp.pair, pp.stop, pp.bookTickerEvent)
}

func (pp *PairProcessor) StartBuyOrSellByDepthSignal() {
	// Запускаємо потік для отримання сигналів на купівлю та продаж
	pp.buy, pp.sell = pp.BuyOrSellByDepthSignal(pp.account, pp.depths, pp.pair, pp.stop, pp.depthEvent)
}
