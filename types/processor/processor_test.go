...
func TestQuantityAndLossCalculation(t *testing.T) {
	...
	percentLiquidation := pp.PercentLiquidation(leverage)
	assert.Equal(t, items_types.PricePercentType(10), percentLiquidation)
	deltaLiquidation := pp.DeltaLiquidation(price, leverage)
	assert.Equal(t, items_types.DeltaPriceType(0.5), deltaLiquidation)

	quantity := pp.PossibleQuantity(transaction, deltaLiquidation)
	assert.Equal(t, items_types.QuantityType(500), quantity)

	loss := pp.PossibleLoss(quantity, items_types.DeltaPriceType(price*items_types.PriceType(percentLiquidation/100)))
	assert.Equal(t, transaction, loss)

	minQuantity := pp.PossibleQuantity(notional, deltaLiquidation)
	assert.Equal(t, items_types.QuantityType(10), minQuantity)
	minLoss := pp.PossibleLoss(minQuantity, items_types.DeltaPriceType(price*items_types.PriceType(percentLiquidation/100)))
...
func TestGetQuantityAndLoss(t *testing.T) {
	...
	deltaLiquidation := pp.DeltaLiquidation(price, leverage)
	assert.Equal(t, items_types.PricePercentType(10), percentLiquidation)
	quantity := pp.PossibleQuantity(value, deltaLiquidation)
