...
func (d *Depth) getIterator(tree *btree.BTree, summa, max, min *types.QuantityType, f ...types.DepthFilter) func(i btree.Item) bool {
...
func (d *Depth) GetFilteredByPercentAsks(f ...types.DepthFilter) (tree *btree.BTree, summa, max, min types.QuantityType) {
...
func (d *Depth) GetFilteredByPercentBids(f ...types.DepthFilter) (tree *btree.BTree, summa, max, min types.QuantityType) {
	tree = btree.New(d.degree)
	if len(f) > 0 {
...
func (d *Depth) GetSummaOfAsksFromRange(first, last types.PriceType, f ...types.DepthFilter) (askSumma, max, min types.QuantityType) {
	var filter types.DepthFilter
	if len(f) > 0 {
		filter = f[0]
	} else {
...
func (d *Depth) GetSummaOfBidsFromRange(first, last types.PriceType, f ...types.DepthFilter) (bidSumma, max, min types.QuantityType) {
	var filter types.DepthFilter
	if len(f) > 0 {
		filter = f[0]
	} else {
...
func (d *Depth) RestrictAskUp(price types.PriceType) {
...
func (d *Depth) RestrictBidUp(price types.PriceType) {
	prices := make([]types.PriceType, 0)
...
func (d *Depth) RestrictAskDown(price types.PriceType) {
	prices := make([]types.PriceType, 0)
...
func (d *Depth) RestrictBidDown(price types.PriceType) {
	prices := make([]types.PriceType, 0)
