package depth

// // GetAsk implements depth_interface.Depths.
// func (d *Depths) GetAsk(price types.PriceType) btree.Item {
// 	item := d.asks.Get(types.New(price))
// 	if item == nil {
// 		return nil
// 	}
// 	return item
// }

// // GetBid implements depth_interface.Depths.
// func (d *Depths) GetBid(price types.PriceType) btree.Item {
// 	item := d.bids.Get(types.New(price))
// 	if item == nil {
// 		return nil
// 	}
// 	return item
// }

// // SetAsk implements depth_interface.Depths.
// func (d *Depths) SetAsk(price types.PriceType, quantity types.QuantityType) (err error) {
// 	if old := d.asks.Get(types.New(price)); old != nil {
// 		d.asksSummaQuantity += quantity - old.(*types.DepthItem).GetQuantity()
// 	} else {
// 		d.asksSummaQuantity += quantity
// 		d.asksCountQuantity++
// 	}
// 	d.asks.ReplaceOrInsert(types.New(price, quantity))
// 	d.AddAskMinMax(price, quantity)
// 	err = d.AddAskNormalized(price, quantity)
// 	return
// }

// // SetBid implements depth_interface.Depths.
// func (d *Depths) SetBid(price types.PriceType, quantity types.QuantityType) (err error) {
// 	if old := d.bids.Get(types.New(price)); old != nil {
// 		d.bidsSummaQuantity += quantity - old.(*types.DepthItem).GetQuantity()
// 	} else {
// 		d.bidsSummaQuantity += quantity
// 		d.bidsCountQuantity++
// 	}
// 	d.bids.ReplaceOrInsert(types.New(price, quantity))
// 	d.AddBidMinMax(price, quantity)
// 	err = d.AddBidNormalized(price, quantity)
// 	return
// }

// // DeleteAsk implements depth_interface.Depths.
// func (d *Depths) DeleteAsk(price types.PriceType) {
// 	old := d.asks.Get(types.New(price))
// 	if old != nil {
// 		d.asksSummaQuantity -= old.(*types.DepthItem).GetQuantity()
// 		d.asksCountQuantity--
// 		d.DeleteAskMinMax(price, old.(*types.DepthItem).GetQuantity())
// 		d.DeleteAskNormalized(price, old.(*types.DepthItem).GetQuantity())
// 		d.asks.Delete(types.New(price))
// 	}
// }

// // DeleteBid implements depth_interface.Depths.
// func (d *Depths) DeleteBid(price types.PriceType) {
// 	old := d.bids.Get(types.New(price))
// 	if old != nil {
// 		d.bidsSummaQuantity -= old.(*types.DepthItem).GetQuantity()
// 		d.asksCountQuantity--
// 		d.DeleteBidMinMax(price, old.(*types.DepthItem).GetQuantity())
// 		d.DeleteBidNormalized(price, old.(*types.DepthItem).GetQuantity())
// 		d.bids.Delete(types.New(price))
// 	}
// }

// // UpdateAsk implements depth_interface.Depths.
// func (d *Depths) UpdateAsk(price types.PriceType, quantity types.QuantityType) bool {
// 	if quantity == 0 {
// 		d.DeleteAsk(price)
// 	} else {
// 		d.SetAsk(price, quantity)
// 		d.DeleteBid(price)
// 	}
// 	return true
// }

// // UpdateBid implements depth_interface.Depths.
// func (d *Depths) UpdateBid(price types.PriceType, quantity types.QuantityType) bool {
// 	if quantity == 0 {
// 		d.DeleteBid(price)
// 	} else {
// 		d.SetBid(price, quantity)
// 		d.DeleteAsk(price)
// 	}
// 	return true
// }

// // AskCount implements depth_interface.Depths.
// func (d *Depths) AskCount() int {
// 	return d.asksCountQuantity
// }

// // BidCount implements depth_interface.Depths.
// func (d *Depths) BidCount() int {
// 	return d.bidsCountQuantity
// }
