...
func GetKlinesUpdateGuard(klines *kline_types.Klines, source chan *binance.WsKlineEvent, IsFinal bool) (
	...
	go func() {
		for {
			event := <-source
			logrus.Debugf("Spot, WsKlineEvent for %v happened", event.Symbol)
			if IsFinal && !event.Kline.IsFinal {
				continue
			}
			logrus.Debugf("Spot, Kline for %v was filled", event.Symbol)
			if klines.GetTime() >= event.Time {
				continue
			}
			logrus.Debugf("Spot, Kline Time for %v was more than event time", event.Symbol)
